...
const Match = () => {
  ...
  const getComments = async () => {
    try {
      const res = await fetch(`http://localhost:4000/api/v1/getComments`, {
        method: "GET",
      });
  ...
  const getPoll = async () => {
    ...
    data.forEach((poll, i) => {
      if (poll.name === "Salman Khan") {
        if (poll.tid === tid) {
          localStorage.setItem("poll", '0');
        }
        spoll += 1;
      } else {
        if (poll.tid === tid) {
          localStorage.setItem("poll", '1');
        }
        kpoll += 1;
      }
    });
    setPollData([{ name: "Poll", "Salman Khan": spoll, "Kanye West": kpoll }]);
  };
      ...
      {!isAuth ? (
        ...
        <>
          ...
          >
            ...
            >
					<video src="video.mp4" style={{maxWidth:'100%', height:'-webkit-fill-available'}} autoPlay controls></video>
            </div>
            <div
              style={{
                width: "100%",
